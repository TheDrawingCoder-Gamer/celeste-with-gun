...
fn draw(ctx: *anyopaque) void {
    const self: *Slope = @alignCast(@ptrCast(ctx));
    const rot = self.get_rot();
    self.game_object.game_state.draw_sprite(sheets.slope.items[0], self.game_object.x, self.game_object.y, .{ .rotate = rot });
}

fn get_rot(self: *const Slope) tic.Rotate {
...
fn destroy(self: *Slope, allocator: std.mem.Allocator) void {
    allocator.destroy(self);
}

pub fn create(allocator: std.mem.Allocator, state: *GameState, x: i32, y: i32, dir: CardinalDir) !*Slope {
    ...
    obj.special_type = .slope;
    const self = try allocator.create(Slope);
