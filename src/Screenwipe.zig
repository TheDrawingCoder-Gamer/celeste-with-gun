...
pub fn draw(self: *const Self, game_state: *const GameState) void {
    if (self.wipe_timer > 5) {
        const e: f32 = @as(f32, @floatFromInt(self.wipe_timer - 5)) / 12;
        for (0..tic.HEIGHT) |i| {
    ...
    if (self.infade < 15) {
        const e = @as(f32, @floatFromInt(self.infade)) / 12;
        for (0..tic.HEIGHT) |i| {
            const s: i32 = @intFromFloat(@as(f32, tic.WIDTH + 64) * e - 32 + tic80sin(@as(f32, @floatFromInt(i)) * 0.2) * 16 + @as(f32, @floatFromInt(tic.WIDTH - i)) * 0.25);
            tic.rect(game_state.camera_x + tic.WIDTH - s, game_state.camera_y + @as(i32, @intCast(i)), tic.WIDTH, 1, 0);
        }
    }
}
